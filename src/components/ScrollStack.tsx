...
const ScrollStack = ({
  ...
  const updateCardTransforms = useCallback(() => {
    ...
    const { scrollTop, containerHeight, scrollContainer } =
      getScrollData()
    ...
    // Use cached offsets instead of recalculating each frame
    ...
    cardsRef.current.forEach((card, i) => {
      ...
      const hasChanged =
        ...
        Math.abs(lastTransform.scale - newTransform.scale) >
          0.001 ||
        Math.abs(
  ...
  const handleScroll = useCallback(() => {
    if (!animationFrameRef.current && !useSmoothScroll) {
  ...
  }, [updateCardTransforms, useSmoothScroll])

  const setupScrollListener = useCallback(() => {
    if (!useSmoothScroll) {
      // Use native scroll without Lenis
  ...
  useLayoutEffect(() => {
    ...
    // Add unique ID to this ScrollStack instance to prevent selecting cards from other instances
    const instanceId = `scroll-stack-${Math.random().toString(36).substr(2, 9)}`
    scroller.setAttribute(
      'data-scroll-stack-id',
      instanceId,
    )
    ...
    const cards = Array.from(
      useWindowScroll
        ? document.querySelectorAll(
            `[data-scroll-stack-id="${instanceId}"] .scroll-stack-card`,
          )
    ...
    // Cache card offsets to prevent jitter from recalculating positions
    ...
    endElementOffsetRef.current = endElement
    ...
    // Setup either Lenis or native scroll listener
    ...
    return () => {
      ...
      isUpdatingRef.current = false
      scroller.removeAttribute('data-scroll-stack-id')
    }
  }, [
    itemDistance,
    itemScale,
